...
exports.run = async (client, message, args) => {
  ...
  let Yetkili = db.fetch("banyetkili_" + message.guild.id);
  ...
  if (
    ...
    return message.channel.send(
        new Discord.MessageEmbed()
        .setColor("#00ff00")
        .setDescription(`**Ban Yetkim Yok!**`)
    )
    .then(m => m.delete({timeout: 5000}));
  await message.guild.members.ban(Kişi.id, { reason: Sebep });
  await message.guild.channels.cache
    .get(Log)
    .send(
        new Discord.MessageEmbed()
      .setTitle("| BAN Sistem |")
      .setDescription(`**Banlanan Üye:<@${Kişi.id}> \n\nBanlayan Yetkili:<@${message.author.id}>\n\nSebep:${Sebep}**`)
